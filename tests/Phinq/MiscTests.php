...
class MiscTests extends \PHPUnit_Framework_TestCase {
	...
	public function testAggregate() {
		$factorial = Phinq::create(array(1, 2, 3, 4, 5))->aggregate(function($current, $next) {
			return $current * $next;
		}, 1);
	...
	public function testAggregateWithEmptyCollection() {
		$factorial = Phinq::create(array())->aggregate(function($current, $next) {
			return $current * $next;
		}, 1);
	...
	public function testSelectMany() {
		...
		$newCollection = Phinq::create($collection)->selectMany(function($value) {
			return array($value, $value + 3);
		})->toArray();
	...
	public function testSimpleJoin() {
		...
		function($innerValue, $outerValue) {
			return $innerValue;
		}
		)->toArray();
	...
	public function testJoinWithComparer() {
		...
		function($innerValue, $outerValue) {
			return "first: $innerValue->foo, second: $outerValue->foo";
		},
		new IdComparer()
		)->toArray();
	...
	public function testGroupJoin() {
		...
		function($key, array $matches) {
			return array('key' => $key, 'matches' => $matches);
		},
		new IdComparer()
		)->toArray();
	...
	public function testZip() {
		self::assertSame(array('foobar'), Phinq::create(array('foo'))->zip(array('bar'), function($a, $b) {
			return $a . $b;
		})->toArray());
	}

	public function testZipWithDifferentSizeArrays() {
		self::assertSame(array('foobar'), Phinq::create(array('foo', 'bar'))->zip(array('bar'), function($a, $b) {
			return $a . $b;
		})->toArray());
		self::assertSame(array('foobar'), Phinq::create(array('foo'))->zip(array('bar', 'foo'), function($a, $b) {
			return $a . $b;
		})->toArray());
	}
	...
	public function testWalk() {
		...
		self::assertSame(array('foo', 'bar'), Phinq::create(array('foo', 'bar'))->walk(function($value) use (&$temp) {
			$temp[] = $value;
		})->toArray());
